...
function init(args) {
    const QisiCli = chalk.bold.cyan('Qisi CLI')
    ...
    isCn = true
    console.log(QisiCli + (!isCn ? ' is booting... ' : ' 正在启动...'))
    console.log(
        QisiCli +
    ...
    function createApp() {
        console.log(
            chalk.bold.cyan('Qisi CLI') +
      (!isCn
          ? ' will creating a new qisi app in '
          : ' 即将创建一个新的应用在 ') +
      dest
        )

        const {status, error: cloneError} = spawn.sync('git', ['clone', '--depth=1',
         `https://github.com/liudingxi-lemi/qisi-template-${templateName}` , customPrjName || '.'])
